...
import { blogs } from "../data/Blogs";  // Import from Blogs.js
        ...
        {blogs.map((blog) => (
          <Link key={blog.slug} to={`/blog/${blog.slug}`}>
            <article className="card card-hover p-5 h-full">
              <span className="badge mb-4 inline-block">{blog.category}</span>  {/* Use category instead of slug */}
              <h2 className="text-xl font-semibold mb-3">{blog.title}</h2>
              <p className="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
                {blog.excerpt}  {/* Use real excerpt */}
              </p>
